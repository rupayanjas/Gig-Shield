...
import { getRiskLevel } from '../lib/liveHelpers';
...
export default function Safety() {
  ...
  // Live data for trust score and active triggers
  const { liveUser, triggers, loading } = useLiveData(user?.phone);
    ...
    <div className="min-h-screen bg-brand-50 relative flex flex-col">
      ...
      <main className="pt-32 pb-20 px-6 max-w-7xl mx-auto flex-1 w-full flex flex-col gap-10">
        <div>
          ...
          <h1 className="text-5xl font-serif text-brand-900 mb-2">Safety Center</h1>
          <p className="text-brand-800 text-sm">Monitor zone risks, trust score, and active dynamic assessments.</p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            {loading || trustScore === null ? (
              ...
              <>
                <div className="flex items-end gap-2 mb-2 z-10">
                  <span className="text-5xl font-serif text-brand-900">{trustScore}</span>
          ...
          {/* Zone Risk Analysis — LIVE (derived from active triggers) */}
          <Card className="flex flex-col md:col-span-2 bg-white p-6 relative overflow-hidden group">
            ...
            <div className="flex items-center justify-between mb-4 z-10">
              ...
              {loading ? (
                <div className="h-6 w-16 bg-brand-100 rounded animate-pulse" />
              ) : (
                <span className={cn(
            ...
            <div className="w-full bg-brand-50 h-3 rounded-full overflow-hidden mb-2 z-10">
              <div
                className={cn(
                  ...
                  riskLevel === 'MEDIUM' ? 'bg-amber-500' : 'bg-green-400'
                )}
                style={{ width: loading ? '0%' : `${riskProb}%` }}
              />
            </div>
            <p className="text-xs text-brand-500 z-10">
              {loading ? 'Analysing zone…' : `${riskProb}% probability of disruption · ${triggers.length} active event(s) in ${city}`}
            </p>
          </Card>
        </div>

        {/* HARDCODED: Emergency SOS — stays static, no backend needed */}
        <Card className="p-0 overflow-hidden border-t-4 border-t-red-500 bg-white">
          <div className="p-6">
            ...
            <p className="text-sm text-brand-600 mb-6 max-w-2xl">
              If you are facing a severe safety issue or accident while on duty in the {city} area, access priority support immediately.
            </p>
